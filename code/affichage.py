...
I = len(wk)
# on suppose M et N egaux
N = round(sqrt(I) + 1)
M = N
h1 = 2 * a / N
h2 = 2 * b / M
det = h1 * h2
w_eta_h = [[0] * (M + 1) for i in range(N + 1)]
w_eta = [[0] * (M + 1) for i in range(N + 1)]
for k in range(I):
	i = (k % (N - 1)) + 1
	j = (k // (N - 1)) + 1
	xi = a * (2 * i - N) / N
	yj = b * (2 * j - M) / M
	w_eta_h[i][j] = wk[k]
	w_eta[i][j] = solution_exacte(xi, yj) - u_star(xi, yj)
...
def f(x, y):
	...
	return lambdas[0] * w_eta_h[i0][j0] + lambdas[1] * w_eta_h[i1][j1] + lambdas[2] * w_eta_h[i2][j2]
...
ax.plot_surface(np.array(X_a), np.array(Y_a), np.array(w_eta), rstride=1, cstride=1, cmap='viridis', edgecolor='none')
ax.set_title('solution exacte, N, M = ' + str(N))
ax.set_xlabel('x')
ax.set_ylabel('y')
ax.set_zlabel('w_eta(x, y)')
